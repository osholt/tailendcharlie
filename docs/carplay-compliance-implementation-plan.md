...
| Group mini-map | Replace the prohibited framed inset with a **Group overview** that fits every valid rider on the full map. Enter automatically after 15 seconds of a three-mile/three-minute quiet stretch and return with at least 1.5 miles/90 seconds before guidance; also retain the manual system button. | Restores the no-touch group-wide check used on motorcycles without adding a second root-view overlay or hiding an imminent instruction. |
| Persistent TEC badge | Put a short distance-first label such as `TEC 1.2 mi` in the first CarPlay navigation-bar button and keep that system bar visible during an active ride; the full Ride status retains name, ETA, freshness and trend. Use a system alert for a material, time-sensitive safety state. | The most important TEC value survives host truncation and remains continuously glanceable in system-owned UI without a tap. |
